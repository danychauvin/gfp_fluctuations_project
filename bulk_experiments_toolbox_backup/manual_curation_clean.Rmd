...
# Discarding glucose_aa data for this particular experiment
In this experiment, it happens that glucose_aa plates did not run through the system.
In a nutshell, the incubation time was set to 15 minutes for 8 plates, which caused the syste, not to run the two last plates, pl7 and 8, which had glucose_aa inside.
...
## Solution 1: compute the blank OD and the mean_blank_fluo for this experiment

The empty wells are supposed to be bearing the strain name "empty". We can start by looking at what they actually look like.
...
mydata %>%
  ...
  filter(time_min>180) %>% 
  ggplot()+
  geom_point(aes(time_min,od))+
  facet_wrap(~interaction(plate,row,column))
```
Keeping A and E
```{r}
good_blanks <- c("pl5.1.A","pl3.1.C","pl5.1.E")

mydata %>%
  filter(strain=="empty") %>% 
  filter(time_min>180) %>% 
  mutate(well=paste(plate,row,column,sep=".")) %>% 
  filter(well %in% good_blanks) %>% 
  ggplot()+
  geom_point(aes(time_min,od))+
  facet_wrap(~interaction(plate,row,column))
```
...
mydata %>%
  filter(strain=="empty") %>% 
  filter(between(time_min,60*3,35*60)) %>% #35h is enough
  mutate(well=paste(plate,row,column,sep=".")) %>%
  filter(well %in% c("pl5.1.A","pl3.1.C","pl5.1.E")) %>% 
  mutate(mean_blank_od=mean(od,na.rm=TRUE)) %>% 
  mutate(sd_blank_od=sd(od,na.rm=TRUE)) %>% 
  filter(between(od,mean_blank_od-2*sd_blank_od,mean_blank_od-2*sd_blank_od)==FALSE) %>% 
  ggplot()+
  geom_point(aes(time_min,od))+
  facet_wrap(~interaction(plate,row,column))
```
...
mean_blank_fluo <- unique(mydata %>%
  ...
  mutate(sd_blank_od=sd(od,na.rm=TRUE)) %>% 
  filter(between(od,mean_blank_od-2*sd_blank_od,mean_blank_od-2*sd_blank_od)==FALSE) %>% 
  mutate(mean_blank_fluo=mean(fluo,na.rm=TRUE)) %>% 
  .$mean_blank_fluo)
```

Now correcting the data for such od in this dataset
...
## Solution 2: take blank OD from older experiment.
...
for(i in plates){
  mydata_to_plot <- mydata %>% 
    filter(plate==i) %>% 
    filter(time_min/60<30) #30h is enough
...
raw_data <- mydata %>%
  ...
  filter(time_min/60<30) #clearly nothing interesting to see after 30 hours.
```
...
raw_data <- mydata %>%
  filter(condition==cond) %>% #keeping only one relevant condition
  filter(!grepl("empty",strain)) %>% #filter out empty wells
  filter(max(corrected_od)>0.05) %>% #to get rid of wells in which there are no growth at all 
  filter(time_min/60<35) #clearly nothing intersting to see after 30 hours.
```
...
# Saving data
raw_data %>% 
  readr::write_csv("/scicore/home/nimwegen/schdav16/Projects/constitexpr/gluaa_curated.csv")
```
