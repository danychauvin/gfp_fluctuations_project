...
ggsave("./figures/cv_c_versus_time.pdf",width=10,height=5,limitsize = FALSE)
```
...
```
## Checking growth-rate and production over time, one 
...
myframes %>% 
  ...
  ungroup() %>% 
  ggplot()+
  geom_point(aes(time_h_rescaled,mean_lambda),alpha=0.5)+
  theme_cowplot()+
  facet_wrap(~condition,nrow=2,scale="free")+
  ylab("Lambda")+
  xlab("Time (h)")

ggsave("./gr_q_versus_time.pdf",width=10,height=5,limitsize = FALSE)
```

- So it seems to be clear... that we are not in a permanent regime. Glycerol/Glucose/Glucoseaa seem to be ok.
...
myframes %>% 
  ...
  ungroup() %>% 
  ggplot()+
  geom_line(aes(time_h_rescaled,mean_lambda,group=gl_id),alpha=0.5)+
  theme_cowplot()+
  facet_wrap(~condition,nrow=2,scale="free")+
  ylab("Lambda")+
  xlab("Time (h)")
...
Several pathologies.
1 - In glycerol data, lambda is going down between 0 and 10h. Clearly not stationnary. The cells are still experiencing a switch. The switching time should be extended to 12h total.
...
myframes %>% 
  ...
  ungroup() %>% 
  ggplot()+
  geom_point(aes(time_h_rescaled,lambda),alpha=0.1)+
  theme_cowplot()+
  facet_wrap(~condition,nrow=2,scale="free")+
  ylab("Lambda")+
  xlab("Time (h)")
...
- What's happening in... acetate005.
...
myframes %>% 
  ...
  ungroup() %>% 
  ggplot()+
  geom_point(aes(time_h_rescaled,q),alpha=0.1)+
  theme_cowplot()+
  facet_wrap(~condition,nrow=2,scale="free")+
  ylab("Q")+
  xlab("Time (h)")
