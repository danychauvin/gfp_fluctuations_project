...
plot_histogram <- function(.cond,breakVec=seq(4000,43000,500),plot=FALSE){
  ...
  exp_density <- density(noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$concentration)
  sim_density <- density(noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$concentration)
  exp <- noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$concentration
  sim <- noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$concentration
  exp_density
  sim_density
  print(exp_density)
  print(sim_density)
...
plot_histogram <- function(.cond,breakVec=seq(4000,43000,500),plot=FALSE){
  ...
  exp_density <- density(noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$length_um)
  sim_density <- density(noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$length_um)
  exp <- noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$length_um
  sim <- noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$length_um
  exp_density
  sim_density
  print(exp_density)
  print(sim_density)
...
plot_histogram <- function(.cond,breakVec=seq(4000,43000,500),plot=FALSE){
  ...
  exp_density <- density(noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% mutate(ll=log(length_um)) %>%  .$ll)
  sim_density <- density(noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% mutate(ll=log(length_um)) %>%  .$ll)
  exp <- noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% mutate(ll=log(length_um)) %>% .$ll
  sim <- noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% mutate(ll=log(length_um)) %>% .$ll
  exp_density
  sim_density
  print(exp_density)
  print(sim_density)
...
plot_histogram <- function(.cond,breakVec=seq(4000,43000,500),plot=FALSE){
  ...
  exp_density <- density(noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$gfp_nb)
  sim_density <- density(noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$gfp_nb)
  exp <- noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% .$gfp_nb
  sim <- noise_ss %>% filter(condition==.cond,datatype=="noprodnoise") %>% .$gfp_nb
  exp_density
  sim_density
  print(exp_density)
  print(sim_density)
...
plot_histogram <- function(.cond,breakVec=seq(4000,43000,500),plot=FALSE){
  ...
  exp_density <- density(noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% mutate(ll=log(gfp_nb)) %>%  .$ll)
  sim_density <- density(noise_ss %>% filter(condition==.cond,datatype=="allnoisesources") %>% mutate(ll=log(gfp_nb)) %>%  .$ll)
  exp <- noise_ss %>% filter(condition==.cond,datatype=="experimentaldata") %>% mutate(ll=log(gfp_nb)) %>% .$ll
  sim <- noise_ss %>% filter(condition==.cond,datatype=="allnoisesources") %>% mutate(ll=log(gfp_nb)) %>% .$ll
  exp_density
  sim_density
  print(exp_density)
  print(sim_density)
...
- In acetate020, the tail of the distribution is making everything shaky.
- Above concentration of 30000 or so, that is causing the increased variance we witness, in the newly computed CV.
...
bad_cells_point <- noise_ss %>% 
  filter(condition=="acetate020") %>%
  filter(concentration > 30000) %>% 
  distinct(gl_id,cell,time_sec)

# Problematic gl_id are: 20220824.4, 20220824.5, 20220824.6

bad_gls <- unique(bad_gls$gl_id)
```

```{r}
#Let's investigate, gl 3.6
noise_ss %>% 
  filter(!(cell %in% c("20220824.5.22.26","20220824.5.6.35"))) %>% 
  filter(gl_id %in% bad_gls) %>% 
  filter(datatype %in% c("experimentaldata","noprodnoise")) %>% 
  ggplot()+
...
myframes %>% 
  mutate(concentration=gfp_nb/length_um) %>% 
  filter(gl_id=="20220824.6.11") %>% 
  ggplot()+
  geom_line(aes(time_sec/3600,concentration,group=cell))
```



So it is clear, all my problems are coming from there. Some cells have some hiccups, they start growing slowly. Result is that concentration gets crazy.
- In 20220824.5.22, this is pretty obvious, but the cell do grows again after some time. So it is not dead. I therefore do not have good reasons, to discard this cell.
- In 20220824.5.6, same thing is happening.
- In 20220824.4.28 a cell was certainly growing slowly before and gave birth to this particular cell that has very high concentration.
- In 20220824.6.11, same thing is happening.
- These are biological facts, but the whole behavior of these cells in the mother machine prevents us from delivering a clear message. What can I do about it?
- These cells are NOT behaving like any others. It is just two cells in the data set. What happens if I get rid of them?
...
```{r}
#select a relevant lineage, where you see the maximum difference on average, for all conditions
relevant_data <- noise_contribution_data %>% 
  filter(datatype %in% c("experimentaldata","noprodnoise")) %>%
  select(condition,datatype,cell,time_sec,concentration,gl_id) %>% 
  pivot_wider(names_from = datatype,values_from = concentration) %>%
  mutate(delta_c=noprodnoise-experimentaldata) %>% 
  group_by(condition,gl_id) %>%
  mutate(mean_delta_c=mean(delta_c)) %>% 
  ungroup() %>% 
  distinct(condition,gl_id,mean_delta_c,.keep_all = TRUE) %>% 
  group_by(condition) %>% 
  slice_max(mean_delta_c) %>% 
  ungroup() %>% 
  select(gl_id,condition)
```

```{r}
toplot <- noise_contribution_data %>% 
  filter(datatype %in% c("experimentaldata","noprodnoise")) %>%
  select(condition,lambda,q,datatype,cell,time_sec,concentration,gl_id) %>% 
  #pivot_wider(names_from = datatype,values_from = concentration) %>% 
  semi_join(relevant_data,by=c("gl_id","condition"))

toplot %>% 
  ggplot()+
  geom_point(aes(time_sec/3600,concentration,col=datatype,group=interaction(cell,datatype)),alpha=0.5)+
  facet_wrap(~condition,scale="free")

toplot %>% 
  ggplot()+
  geom_point(aes(time_sec/3600,lambda,col=datatype,group=interaction(cell,datatype)),alpha=0.5)+
  facet_wrap(~condition,scale="free")

toplot %>% 
  filter(datatype=="noprodnoise") %>% 
  ggplot()+
  geom_point(aes(time_sec/3600,q,col=datatype,group=interaction(cell,datatype)),alpha=0.5)+
  facet_wrap(~condition,scale="free")
```
...
```{r}
toplot <- noise_contribution_data %>% 
  filter(datatype %in% c("experimentaldata","nogrowthnoise")) %>%
  select(condition,lambda,q,datatype,cell,time_sec,concentration,gl_id) %>% 
  #pivot_wider(names_from = datatype,values_from = concentration) %>% 
  semi_join(relevant_data,by=c("gl_id","condition"))
...
glucose_data %>% 
  ...
  pivot_wider(names_from = datatype,values_from = concentration) %>% 
  mutate(delta_c=noprodnoise-experimentaldata) %>% 
  mutate(delta_c=(noprodnoise-experimentaldata)/experimentaldata) %>% 
  ggplot()+
  geom_point(aes(time_sec/3600,delta_c),alpha=0.01)+
  ylab("noprodnoise - experimentaldata")+
  xlab("time (h)")
```
...
h <- hist(delta_c_data,breaks=seq(-12000,23237,100),plot=FALSE)
