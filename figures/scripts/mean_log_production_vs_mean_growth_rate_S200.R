...
data_toplot <- experimental_data %>% 
  ...
  mutate(ir=q75-q25) %>% 
  ungroup() %>% 
  distinct(condition,promoter,replica,date,mean_gr,sd_gr,mean_x,sd_x) %>% 
  mutate(date=as.character(date))

data_toplot %>% 
  left_join(label_df,by=c("condition")) %>% 
  ggplot()+
  geom_point(aes(mean_gr,mean_x,col=date),alpha=0.5)+
  geom_errorbar(aes(x=mean_gr,ymax=mean_x+sd_x,ymin=mean_x-sd_x,col=date),alpha=0.5)+
...
ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/fig2/mean_log_production_vs_mean_growth_rate_S200.pdf",width=6,height=4)