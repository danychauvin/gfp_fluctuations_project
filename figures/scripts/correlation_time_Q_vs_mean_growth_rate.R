gamma_q_df <- myparameters %>% 
  ...
  mutate(gamma_q_normalized=gamma_q*(mean_doubling_time*60)) %>% #minutes
  select(condition,promoter,date,gamma_q,gamma_q_normalized)

gamma_q_err_df <- myerrors %>% 
  filter(epsilon==0.05) %>% 
  mutate(gamma_q_err=sqrt(gamma_q)) %>% 
  select(condition,promoter,date,gamma_q_err)
...
toplot <- left_join(gamma_q_df,gamma_q_err_df,by=c("condition","date","promoter")) %>% 
  left_join(mean_lambda_df,by=c("condition","date","promoter")) %>% 
  left_join(mean_lambda_err_df,by=c("condition","date","promoter"))

toplot %>% 
  #filter(1/gamma_q<1500) %>% 
  mutate(autocor_time=1/gamma_q,
         sd_autocor_time=abs(1/(gamma_q**2)*gamma_q_err)) %>% 
  ggplot()+
  ...
  ylab("Production autocorrelation time (min)")

ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/fig2/correlation_time_q_vs_mean_growth_rate_Cdate.pdf",width=8,height=5)
...
ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/fig2/correlation_time_q_vs_mean_growth_rate_Cpromoter.pdf",width=8,height=5)
