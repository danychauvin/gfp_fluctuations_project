...
data_toplot <- experimental_data %>% 
  ...
  distinct(condition,promoter,replica,date,cv_gr,mean_gr)

data_toplot %>% 
  left_join(label_df,by=c("condition")) %>% 
  ggplot()+
  geom_point(aes(mean_gr,cv_gr,col=promoter,shape=replica),alpha=0.5)+
...
ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/fig2/cv_growth_rate_vs_mean_growth_rate_S200.pdf",width=6,height=4)