...
data_toplot <- experimental_data %>% 
  ...
  mutate(ir=q75-q25) %>% 
  ungroup() %>% 
  distinct(condition,promoter,replica,date,mean_x,sd_x,mean_gr,sd_gr,ir)
...
data_toplot %>% 
  ...
  ylab("Interquartile range log-GFP")+
  #scale_y_continuous(limits=c(0,0.8))+
  scale_x_continuous(limits=c(0,0.030))+
  theme(text = element_text(size = 8),axis.text = element_text(size = 5),
        legend.spacing.y = unit(0.05, 'cm'))

ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/figures/pdfs/ir_tot_GFP_vs_mean_growth_rate.pdf",width=6,height=4)