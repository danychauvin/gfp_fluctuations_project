...
data_toplot %>% 
  ...
  ggplot()+
  geom_point(aes(mean_gr,mean_x),alpha=0.5)+
  geom_errorbar(aes(x=mean_gr,ymax=mean_x+sd_x,ymin=mean_x-sd_x),alpha=0.5)+
  geom_errorbar(aes(y=mean_x,xmax=mean_gr+sd_gr,xmin=mean_gr-sd_gr),alpha=0.5)+
...
ggsave("/scicore/home/nimwegen/rocasu25/Documents/Projects/biozentrum/gfp_fluctuations_project/fig2/mean_log_c_vs_mean_growth_rate_Fpromoters_S200.pdf",width=6,height=4)