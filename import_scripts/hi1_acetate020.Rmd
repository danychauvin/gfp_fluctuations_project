...
# Number of growth lanes, number of positions
...
# Total number of cells in each growth lane, stratified by rank

```{r}
plot_dataset_statistics(.condition,.promoter,FALSE)
```

# Total number of cells in each growth lane, stratified by rank, complete cell cycle only
...
# Computing mean growth rate and mean production for inference
...
The following shows how many independent cells are within the curated data-set.
```{r}
plot_dataset_statistics_curated(.condition,.promoter,TRUE)
```

# Number of cells in raw curated data
...
So there is some stratification of the growth rate, and mother cells do not grow "exponentially". Pos0 is actually worse than position 1, that's worse than position 2.
What do I do with this? Which are the cells characterized by a bad r2?
...
# Distribution of added volume and length at birth, scatter plot
...
# Growth rate over the course of the experiment
...
# GFP variables: checking for the tilt or other abnormalities
...
# Checking for the all cells case, that cell_detection_offset has been set properly
