...
myframes <- myframes %>%
  ...
  tidyr::extract(file_path, c("date","pos", "gl"), "__(\\d{8})[glugly]*_.*[Pp]os(\\d+)_GL(\\d+).*.csv$", remove=FALSE, convert=TRUE) %>%
...
myframes <- myframes %>%
  group_by(cell) %>% 
  arrange(frame) %>% 
  mutate(switch_frame=ifelse((time_sec-t_start)<2*3600,TRUE,FALSE)) %>% 
