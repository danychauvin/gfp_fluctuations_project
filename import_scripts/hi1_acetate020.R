...
path_to_MM_data_summary <- "./datalist_hi1_acetate020.csv"
...
## ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# List of cells with obviously wrong segmentation at the end of their cell cycle
...
cells_discarded_manually <- c("6.14.4","4.46.44")
...
myframes_complete_goodcells %>% 
  ggplot()+
  geom_point(aes(frame,log(length_um_oriented_bbox),col=vertical_top)) +
  facet_wrap(~cell,scale="free")

ggsave(sprintf("../raw_curated_data_complete_cycles/%s_%s.pdf",.promoter,.condition),width=50,height=50,limitsize=FALSE)}
...
ggsave("./cell_detection_offset_acetate020.pdf",height=20,width=20)
...
## ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
myframes_all %>% 
  mutate(cell_detection_offset=as.double(cell_detection_offset)) %>% 
  group_by(cell) %>% 
  mutate(touchtop=ifelse(any(vertical_top==cell_detection_offset,na.rm=TRUE),TRUE,FALSE)) %>% 
  ungroup() %>% 
  filter(touchtop==TRUE) %>% 
  ggplot()+
  geom_point(aes(-vertical_top,length_um))+
  facet_wrap(~cell)
  #facet_wrap(~paste(cell,condition),scale="free")
...
ggsave("./length_time_acetate005.pdf",height=20,width=20)
