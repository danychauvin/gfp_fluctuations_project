...
path_to_MM_data_summary <- "./datalist_hi1_glucoseaa020.csv"
...
cells_discarded_manually <- c("1.2.175","1.48.128","1.48.121")
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
cells_discarded_manually <- c("1.2.175","1.48.128")
...
mean_exponential_gr_min <- unique(myframes_complete %>% 
  ...
  distinct(cell,.keep_all=TRUE) %>% 
  ungroup() %>% 
  mutate(mean_exp_gr=mean(alpha_oriented_bbox*60,na.rm=TRUE)) %>% 
  .$mean_exp_gr)
...
mean_concentration <- unique(myframes_complete %>% 
  ...
  ungroup() %>% 
  group_by(cell) %>% 
  mutate(mean_concentration=mean(gfp_nb/length_um_oriented_bbox,na.rm=TRUE)) %>% 
  ungroup() %>% 
  distinct(cell,.keep_all=TRUE) %>% 
...
for (i in c(0:9)){
  good_cells_fraction <- good_cells %>% 
    filter(dplyr::between(row_number(),nrow(good_cells)/10*i,nrow(good_cells)/10*(i+1))==TRUE)
  ...
  ggsave(sprintf("../raw_curated_data_complete_cycles/%s_%s_%i_6.pdf",.promoter,.condition,i),width=50,height=50,limitsize=FALSE)}}
...
mycells_complete_goodcells %>% 
  ...
  mutate(sd_gr=sd(alpha_oriented_bbox*3600/log(2),na.rm=TRUE)) %>% 
...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
unique(mycells_complete %>% 
  semi_join(good_cells,by=c("cell")) %>% 
  distinct(cell,.keep_all=TRUE) %>% 
  ungroup() %>% 
  mutate(mean_doubling_time=mean(1/(alpha_oriented_bbox*3600/log(2)),na.rm=TRUE)) %>% 
  .$mean_doubling_time)
...
#ggsave("./hi1_glucose2aa_growth_rate_vs_time_all_gl.pdf",width=20,height=50,limitsize=FALSE)
...
ggsave("./cell_detection_offset_glucoseaa020.pdf",height=20,width=20)
...
ggsave("./length_time_glucoseaa020.pdf",height=20,width=20)
